...
#define PHYT_I2S_V2_VERSION "1.0.7"
...
static int phyt_pcm_new(struct snd_soc_pcm_runtime *rtd)
{
	struct phytium_i2s *priv = snd_soc_dai_get_drvdata(rtd->cpu_dai);
...
static void phyt_pcm_free(struct snd_pcm *pcm)
{
...
static int phyt_pcm_close(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct phytium_i2s *priv = snd_soc_dai_get_drvdata(rtd->cpu_dai);
...
static int phyt_pcm_hw_params(struct snd_pcm_substream *substream,
...
static int phyt_pcm_prepare(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct phytium_i2s *priv = snd_soc_dai_get_drvdata(rtd->cpu_dai);
	struct snd_pcm_runtime *runtime = substream->runtime;
	...
	phyt_writel_reg(priv->dma_reg_base, PHYTIUM_DMA_CHALX_DLENTH(direction), D_LENGTH);

	return 0;
}

static int phyt_pcm_hw_free(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct phytium_i2s *priv = snd_soc_dai_get_drvdata(rtd->cpu_dai);
	int direction;
...
static int phyt_pcm_trigger(struct snd_pcm_substream *substream, int cmd)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct phytium_i2s *priv = snd_soc_dai_get_drvdata(rtd->cpu_dai);
	bool start =  false;
...
static snd_pcm_uframes_t phyt_pcm_pointer(struct snd_pcm_substream *substream)
{
	struct snd_soc_pcm_runtime *rtd = substream->private_data;
	struct phytium_i2s *priv = snd_soc_dai_get_drvdata(rtd->cpu_dai);
	uint32_t pos = 0;
...
static int phyt_pcm_suspend(struct snd_soc_dai *dai)
{
	return 0;
}

static int phyt_pcm_resume(struct snd_soc_dai *dai)
{
					    ...
					    struct snd_soc_dai_driver *dai_driver)
{
	...
	dai_driver->suspend = phyt_pcm_suspend;
	dai_driver->resume = phyt_pcm_resume;
	return 0;
}
...
void phyt_i2s_show_log(struct phytium_i2s *priv)
{
	...
	plog = priv->log_addr;
	if (reg & LOG_MASK) {
		len = strnlen((char *)priv->log_addr, 8192);

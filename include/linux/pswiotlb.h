...
void pswiotlb_sync_single_for_device(struct device *dev, int nid,
			...
			enum dma_data_direction dir, struct p_io_tlb_pool *pool);
...
static bool is_ps23064_socs;
static inline bool is_phytium_ps23064_socs(void)
{
	unsigned int soc_id;
	unsigned int midr;

	if (likely(is_ps23064_socs))
		return true;

	soc_id = read_sysreg_s(SYS_AIDR_EL1);
	midr = read_cpuid_id();
	if (soc_id == SOC_ID_PS23064 && midr == MIDR_PS23064) {
		is_ps23064_socs = true;
		return true;
	} else
		return false;
}

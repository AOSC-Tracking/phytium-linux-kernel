...
#define YTPHY_LINUX_VERSION "2.2.42319"
...
static ssize_t phydrv_ver(struct file *file, char __user *user_buf,
			  size_t count, loff_t *ppos) {
	char buf[64];
	int len;

	len = snprintf(buf, sizeof(buf), "Driver Version: %s\n",
		       YTPHY_LINUX_VERSION);
...
static int __init ytphy_init(void)
{
	...
	/* Create debugfs file */
	if (!debugfs_create_file(PHYDRV_VER, 0444, dir, NULL, &ver_fops)) {

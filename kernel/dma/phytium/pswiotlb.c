...
{
	if (isdigit(*str)) {
		/* avoid tail segment of size < P_IO_TLB_SEGSIZE */
		default_npslabs =
			ALIGN(simple_strtoul(str, &str, 0), P_IO_TLB_SEGSIZE);
	}
	if (*str == ',')
		++str;
	if (isdigit(*str))
		pswiotlb_adjust_nareas(simple_strtoul(str, &str, 0));
	if (*str == ',')
		++str;
		...
		int (*remap)(void *tlb, unsigned long npslabs))
{
	...
	tlb = memblock_virt_alloc_try_nid_nopanic(bytes, PAGE_SIZE,
				BOOTMEM_LOW_LIMIT, BOOTMEM_ALLOC_ACCESSIBLE, nid);
	...
	if (remap && remap(tlb, npslabs) < 0) {
		memblock_free_early(__pa(tlb), PAGE_ALIGN(bytes));
		...
		int (*remap)(void *tlb, unsigned long npslabs))
{
	...
	mem->slots = memblock_virt_alloc(alloc_size, PAGE_SIZE);
	if (!mem->slots) {
	...
	mem->areas = memblock_virt_alloc(array_size(sizeof(struct p_io_tlb_area),
		...
		unsigned int alloc_align_mask)
{
	...
	dma_addr_t tbl_dma_addr =
		__phys_to_dma(dev, pool->start) & boundary_mask;
		...
		enum dma_data_direction dir, unsigned long attrs)
{
	...
	pswiotlb_addr = pswiotlb_tbl_map_single(dev, nid, paddr, size,
				PAGE_ALIGN(size), PAGE_SIZE - 1, dir,
			attrs);
	...
	if (!is_device_dma_coherent(dev) && !(attrs & DMA_ATTR_SKIP_CPU_SYNC))
		__dma_map_area(phys_to_virt(pswiotlb_addr), size, dir);
	return dma_addr;
}
